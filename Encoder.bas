...
'
' cmdline "c:\Freebasic_Path\fbc.exe" -x "Encoder.exe" Encoder.bas -s gui -v Encoder.rc > Encoder.log 2>&1

'needed files to compile : this file and Encoder.rc ; Encoder.Exe.Manifest ; import.ico (have them in the same folder as Encoder.bas)
...
_:[CSED_COMPIL_NAME]:  Encoder_last.exe
_:[CSED_COMPIL_RC]:    Encoder.rc
...
' warning need static lib libMemDll_64.a or libMemDll_32.a to encode to file, or make your own version
...
Private Function FORM1_WM_CREATE(ByVal hWndForm as HWnd , _ ' handle of Form
    ...
    FF_ListView_InsertColumn lv , 0 , "Source File(s) Input" , LVCFMT_LEFT , 480
    FF_ListView_InsertColumn lv , 1 , "Destination for Encoded File(s)" , LVCFMT_LEFT , 480
...
Function FORM1_COMMAND1_BN_CLICKED( _
    ...
    If FF_OpenFileDialog(hWndForm , "Select files to encode" , sFilename , _
            Exepath() , _
            "All Files (*.*)|*.*" , _
            "" , _
...
Private Function fill_array(ByRef s0 as String) as Long
    ...
    If k = 1 Then
        ...
        FF_ListView_SetItemText(lv , ipos , 1 , mess1 & mess2 & ".enc")
...
Function FORM1_COMMAND4_BN_CLICKED( _
    ...
    For i = 0 To k - 1
        If FF_ListView_GetItemText(lv , i , 2) = "  --" Then
            ...
            sret = FileEncrypt(FF_ListView_GetItemText(lv , i , 0) , FF_ListView_GetItemText(lv , i , 1) , 0)
            If sret = "" Then
...
Private Function FORM1_COMMAND5_BN_CLICKED( _
    ...
    If k <> - 1 And (FF_ListView_GetItemText(lv , k , 2) = "Existing") Then
        Select Case MessageBox(hWndForm , "" + Chr(10 , 10 , 10) + "Warning, this will overwrite the destination existing file! " + Chr(10 , 10) + "Are you sure, you want to replace that file?" , "Encode" , _
                        MB_YESNO Or MB_ICONQUESTION Or MB_DEFBUTTON2 Or MB_SYSTEMMODAL)
            Case IDYES
                sret = FileEncrypt(FF_ListView_GetItemText(lv , k , 0) , FF_ListView_GetItemText(lv , k , 1) , 1)
                If sret = "" Then
...
Private Function FORM1_COMMAND6_BN_CLICKED( _
    ...
    For i = 0 To k - 1
        FF_Control_SetText(HWND_FORM1_LABEL1 , "doing ...  " & FF_ListView_GetItemText(lv , i , 0))
        sret = FileEncrypt(FF_ListView_GetItemText(lv , i , 0) , FF_ListView_GetItemText(lv , i , 1) , 1)
        If sret = "" Then
            FF_ListView_SetItemText(lv , i , 2 , "Failed")
        ElseIf sret = "__FILE_FOUND__" Then
            FF_ListView_SetItemText(lv , i , 2 , "Existing")
    ...
    If lpcmdline <> "" Then
        ...
        If ss1 <> "" Then
            ...
            Else
                Dim as String sret
                If iforce = 1 Then
                    sret = FileEncrypt(ss1 , ss2 , 1)
                Else
                    sret = FileEncrypt(ss1 , ss2)
                End If
                If sret = "" Then
                    If iflag = 1 Then MessageBox 0 , Chr(10 , 10) & ss1 & Chr(10 , 10) & "No file to encode, exit now!" , "Error" , MB_OK + MB_ICONERROR Or MB_SYSTEMMODAL
                ElseIf sret = "__FILE_FOUND__" Then
                    Select Case MessageBox(0 , Chr(10 , 10) & "Be carefull " & Chr(10 , 10) & "Destination file already exists," & Chr(10) & "do you want to replace it?" , "Encode" , _
                                    MB_YESNO Or MB_ICONWARNING Or MB_DEFBUTTON1 Or MB_SYSTEMMODAL)
                        Case IDYES
                            sret = FileEncrypt(ss1 , ss2 , 1)
                            If iflag = 1 Then MessageBox 0 , Chr(10 , 10) & ss1 & Chr(10 , 10) & "File encoded to :" & Chr(10 , 10) & sret , "Done" , MB_OK + MB_ICONINFORMATION Or MB_SYSTEMMODAL
                        Case IDNO
                            If iflag = 1 Then MessageBox 0 , Chr(10 , 10) & ss1 & Chr(10 , 10) & "File not encoded!" , "Not Done" , MB_OK + MB_ICONWARNING Or MB_SYSTEMMODAL
                    End Select

                Else
                    If iflag = 1 Then MessageBox 0 , Chr(10 , 10) & ss1 & Chr(10 , 10) & "File encoded to :" & Chr(10 , 10) & sret , "Error" , MB_OK + MB_ICONINFORMATION Or MB_SYSTEMMODAL
                End If
                Function = 1
                Exit Function
            End If
        Else
            If iflag = 1 Then MessageBox 0 , Chr(10 , 10) & ss1 & Chr(10 , 10) & "Need file path to encode, exit now! " , "Error" , MB_OK + MB_ICONERROR Or MB_SYSTEMMODAL
            Function = 1
            Exit Function
        End If
    End If
