...
' Generated: Feb 11, 2019 at 11:47:49
' Visit www.planetsquires.com for more information
'------------------------------------------------------------------------------
'*****************************************************************************
' code adapted/simplified for that usage by Marc Pons marpon@aliceadsl.fr
'*****************************************************************************


' to compile that project (via command line ...)
' replace c:\Freebasic_Path\fbc.exe by your path to fbc.exe
'
' cmdline "c:\Freebasic_Path\fbc.exe" -x "Decoder.exe" Decoder.bas -s gui -v Decoder.rc > Decoder.log 2>&1

'needed files to compile : this file and Decoder.rc ; Decoder.Exe.Manifest ; import.ico (have them in the same folder as Decoder.bas)
' static lib : LibMemDll_32.a for 32bits or LibMemDll_64.a for 64bits depending on 32/64 compiler

' the static lib for the functions Encrypt ; read file and write to file , you can replace by your own functions to avoid the lib

'CSED_FB : internal cmds used by CSED_FB editor only, code seen as comment by compiler. So no effect
_:[CSED_COMPIL_NAME]:  Decoder.exe
_:[CSED_COMPIL_RC]:    Decoder.rc
...
Private Function get_extension(ByRef Src As String , byref ipos as long = 0) As String
    ipos = InStrrev(Src , ".")
    If ipos Then
        Function = Mid(Src , ipos + 1)
    Else
        Function = ""
    End If
End Function

Private Function FileIsFound(byref mes As String) As long
    Dim As Integer myHandle , result
...
' warning need static lib libMemDll_64.a or libMemDll_32.a to decode to file, or make your own version
Private Function FileDecrypt(byref Source_File as String, byref Desti_File as String = "", byval iforced as long = 0) as string
	if Source_File = "" THEN return ""
	if Desti_File = "" THEN
		dim as string sname = Source_File
		dim as long ipos = InStrrev( lcase(sname), ".enc")
		dim as string sext
		if ipos = len(sname) - 3 THEN
			dim as long ipos2 = InStrrev( sname, "_")
			if ipos2 > 0 and ipos2 < ipos  THEN
				sext = mid(sname,ipos2 + 1 , ipos - ipos2 - 1)
				sname = left(Source_File, ipos2 - 1) & sext
			else
				sname = left(Source_File, ipos - 1) & "_dec.xxx"
			END IF
		else
			sname = sname & "_dec.xxx"
        END IF
		Desti_File = sname
	END IF
	if FileIsFound(Desti_File) <> 0 and iforced = 0 THEN return "__FILE_FOUND__"
	dim as long ilen
    dim as zstring ptr ps1 = R_File(strptr(Source_File) , @ilen)
    if ilen = 0 or ps1 = NULL THEN return ""
    dim as zstring ptr pbuf = Decrypt(ps1 , ilen)
    if pbuf = NULL THEN return ""
    W_File(strptr(Desti_File) , pbuf , ilen)
    deallocate pbuf
    deallocate ps1
	return Desti_File
End Function

 /' Private Function FileEncrypt(byref Source_File as String , byref Desti_File as String = "" , byval iforced as long = 0) as string
    if Source_File = "" THEN return ""
    if Desti_File = "" THEN
        dim as long ipos
        dim as string sext = get_extension(Source_File , ipos)
        dim as string sname = Source_File
        if ipos THEN
            sname[ipos - 1] = 95
        else
...
Private Function FORM1_Show(ByVal hWndParent As HWND , _
        ...
        if gSHOW = 0 then                        'to block form creation used by dll
    ...
    If IsInitialized = 0 Then
        ...
        wce.hIcon = LoadImage(App.hInstance , "IMAGE_IMPORT" , IMAGE_ICON , 32 , 32 , LR_SHARED)
        ...
        wce.hIconSm = LoadImage(App.hInstance , "IMAGE_IMPORT" , IMAGE_ICON , 16 , 16 , LR_SHARED)
...
Private Sub FORM1_CreateControls(ByVal hWndForm As HWND)
    ...
    hWndControl = CreateWindowEx(WS_EX_LEFT Or WS_EX_LTRREADING , _
            ...
            afxScaleX(1072) , afxScaleY(FLY_ClientOffset + 240) , _
    ...
    hWndControl = CreateWindowEx(WS_EX_LEFT Or WS_EX_LTRREADING , _
            ...
            afxScaleX(392) , afxScaleY(FLY_ClientOffset + 424) , _
    ...
    hWndControl = CreateWindowEx(WS_EX_LEFT Or WS_EX_LTRREADING , _
            ...
            Or BS_PUSHBUTTON Or BS_NOTIFY Or BS_CENTER Or BS_VCENTER _
             , _
            afxScaleX(1072) , afxScaleY(FLY_ClientOffset + 328) , _
    ...
    hWndControl = CreateWindowEx(WS_EX_LEFT Or WS_EX_LTRREADING , _
            ...
            Or BS_PUSHBUTTON Or BS_NOTIFY Or BS_CENTER Or BS_VCENTER _
             , _
            afxScaleX(920) , afxScaleY(FLY_ClientOffset + 416) , _
    ...
    hWndControl = CreateWindowEx(WS_EX_LEFT Or WS_EX_LTRREADING , _
            ...
            Or BS_PUSHBUTTON Or BS_NOTIFY Or BS_CENTER Or BS_VCENTER _
             , _
            afxScaleX(224) , afxScaleY(FLY_ClientOffset + 416) , _
...
Private Function FORM1_WM_CREATE(ByVal hWndForm as HWnd , _ ' handle of Form
    ...
    FF_ListView_InsertColumn lv , 0 , "Source File(s) Encoded Input" , LVCFMT_LEFT , 480
    FF_ListView_InsertColumn lv , 1 , "Destination for Decoded File(s)" , LVCFMT_LEFT , 480
...
Function FORM1_COMMAND1_BN_CLICKED ( _
   ...
   If FF_OpenFileDialog( hWndForm, "Select files to decode", sFilename, _
                         Exepath(), _
                         "Encoded files (*.enc)|*.enc|All Files (*.*)|*.*", _
                         ".enc", _
...
'--------------------------------------------------------------------------------
Function fill_array(ByRef s0 as String) as Long
    ...
    If j Then
     ...
     If k = 1 Then
		...
		mess3 = get_extension(mess2 ,ipos)
		if lcase(mess3) = "enc" and ipos <> 0 THEN
			mess2 = Left(mess2, Len(mess2) - (Len(mess2) - ipos + 1))
			v = InStrRev( mess2,"_")
			if v <> 0 then mess2[v-1] = 46
		else
			mess2 &= "_dec.xxx"
		end if
        ipos = FF_ListView_InsertItem (lv, i+1, 0, s0)
        FF_ListView_SetItemText( lv, ipos , 1, mess1 & mess2 )
...
Function FORM1_COMMAND4_BN_CLICKED( _
    ...
    Dim as String sret
    For i = 0 To k - 1
        If FF_ListView_GetItemText(lv , i , 2) = "  --" Then
            FF_Control_SetText(HWND_FORM1_LABEL1 , "doing ...  " & FF_ListView_GetItemText(lv , i , 0))

            sret = FileDecrypt(FF_ListView_GetItemText(lv , i , 0) , FF_ListView_GetItemText(lv , i , 1) , 0)
            If sret = "" Then
                FF_ListView_SetItemText(lv , i , 2 , "Failed")
            ElseIf sret = "__FILE_FOUND__" Then
...
Private Function FORM1_COMMAND5_BN_CLICKED( _
    ...
    If k <> - 1 And (FF_ListView_GetItemText(lv , k , 2) = "Existing") Then
        Select Case MessageBox(hWndForm , "" + Chr(10 , 10 , 10) + "Warning, this will overwrite the destination existing file! " + Chr(10 , 10) + "Are you sure, you want to replace that file?" , "Decode" , _
                        MB_YESNO Or MB_ICONQUESTION Or MB_DEFBUTTON2 Or MB_SYSTEMMODAL)
            Case IDYES
                sret = FileDecrypt(FF_ListView_GetItemText(lv , k , 0) , FF_ListView_GetItemText(lv , k , 1) , 1)
                If sret = "" Then
...
Private Function FORM1_COMMAND6_BN_CLICKED( _
    ...
    For i = 0 To k - 1
        FF_Control_SetText(HWND_FORM1_LABEL1 , "doing ...  " & FF_ListView_GetItemText(lv , i , 0))
        sret = FileDecrypt(FF_ListView_GetItemText(lv , i , 0) , FF_ListView_GetItemText(lv , i , 1) , 1)
        If sret = "" Then
            FF_ListView_SetItemText(lv , i , 2 , "Failed")
        ElseIf sret = "__FILE_FOUND__" Then
            FF_ListView_SetItemText(lv , i , 2 , "Existing")
...
Private Function FF_WINMAIN(ByVal hInstance as HINSTANCE , _
    ...
    If lpcmdline <> "" Then
        ...
        If LCase(ss1) = "-v" Then
            ...
            ss1 = Command(2)
            ss2 = Command(3)
        ElseIf LCase(ss1) = "-s" Then
            ss1 = Command(2)
            ss2 = Command(3)
        ElseIf LCase(ss1) = "-f" Then
            ss1 = Command(2)
            ss2 = Command(3)
            iforce = 1
        End If
        If ss1 <> "" Then
            If FileIsFound(ss1) = 0 Then
                If iflag = 1 Then MessageBox 0 , Chr(10 , 10) & ss1 & Chr(10 , 10) & "File not found!" , "Error" , MB_OK + MB_ICONERROR Or MB_SYSTEMMODAL
            Else
                Dim as String sret
                If iforce = 1 Then
                    sret = FileDecrypt(ss1 , ss2 , 1)
                Else
                    sret = FileDecrypt(ss1 , ss2)
                End If
                If sret = "" Then
                    If iflag = 1 Then MessageBox 0 , Chr(10 , 10) & ss1 & Chr(10 , 10) & "No file to decode, exit now!" , "Error" , MB_OK + MB_ICONERROR Or MB_SYSTEMMODAL
                ElseIf sret = "__FILE_FOUND__" Then
                    Select Case MessageBox(0 , Chr(10 , 10) & "Be carefull " & Chr(10 , 10) & "Destination file already exists," & Chr(10) & "do you want to replace it?" , "Decode" , _
                                    MB_YESNO Or MB_ICONWARNING Or MB_DEFBUTTON1 Or MB_SYSTEMMODAL)
                        Case IDYES
                            sret = FileDecrypt(ss1 , ss2 , 1)
                            If iflag = 1 Then MessageBox 0 , Chr(10 , 10) & ss1 & Chr(10 , 10) & "File decoded to :" & Chr(10 , 10) & sret , "Done" , MB_OK + MB_ICONINFORMATION Or MB_SYSTEMMODAL
                        Case IDNO
                            If iflag = 1 Then MessageBox 0 , Chr(10 , 10) & ss1 & Chr(10 , 10) & "File not decoded!" , "Not Done" , MB_OK + MB_ICONWARNING Or MB_SYSTEMMODAL
                    End Select

                Else
                    If iflag = 1 Then MessageBox 0 , Chr(10 , 10) & ss1 & Chr(10 , 10) & "File decoded to :" & Chr(10 , 10) & sret , "Error" , MB_OK + MB_ICONINFORMATION Or MB_SYSTEMMODAL
                End If
                Function = 1
                Exit Function
            End If
        Else
            If iflag = 1 Then MessageBox 0 , Chr(10 , 10) & ss1 & Chr(10 , 10) & "Need file path to decode, exit now! " , "Error" , MB_OK + MB_ICONERROR Or MB_SYSTEMMODAL
            Function = 1
            Exit Function
        End If
    End If
