...
this bas file   mem_exe_dll_light2.bas
...
'put under the files you want to test according 64 or 32 bits, already encoded with encoder.exe
#ifdef __FB_64BIT__
	#define _ExeFullPath "test64_exe.enc"
	#define _DllFullPath "my_dll64_dll.enc"
#else
	#define _ExeFullPath "test32_exe.enc"
	#define _DllFullPath "Calendrier_dll.enc"
#endif


#ifndef __MEM_EXE_DLL_HEADER__
    #define __MEM_EXE_DLL_HEADER__
    #ifdef __FB_64BIT__
        #INCLIB "MemExeDll_64"                      ' C lib64 > libMemExeDll_64.a
    #else
        #INCLIB "MemExeDll_32"                      ' C lib32 > libMemExeDll_32.a
    #endif
	...
	EXTERN "C"
		...
		'========================================================
		DECLARE FUNCTION ExecMemExe alias "C_Fork"(byval dataU as any ptr, byref cmdline as zstring = "", byval ifConsoleshow as long = 0) as Ulong  'return pid

		DECLARE FUNCTION Exe_Memory alias "C_ExeMem"(byval dataU as any ptr) as Ulong  'return pid

		DECLARE FUNCTION Exe_WaitMemory alias "C_ExeWaitMem"(byval dataU as any ptr, byref cmdline as zstring = "" , byval ifConsoleshow as long = 0) as Ulong ' execute and return to the line after when finish the exe
        ...
        DECLARE FUNCTION Decrypt alias "Decrypt"(byval input as zstring ptr , byval ilen as long) as zstring ptr
	END EXTERN
	...
	Private	Function ExecWaitMem overload ( byval pdata as any ptr, byref str_cmd as zstring = "", byval ifConsoleshow as long = 0) As Ulong
	    dim as Ulong pid
...
' to show usage
	...
	sub dll_32()
		...
		if Date1 = 0 THEN
			...
			messagebox 0, Date1(06, 5, 1985), "Selected date", MB_OK
		end if
	    DllMemoryFree(hhdll)
...
sub exe_64_32()
	...
	dim as ubyte ptr p1 = allocate(EXE64_32_len)
	p1 = Decrypt(EXE64_32, EXE64_32_len)
	memcpy(EXE64_32, p1, EXE64_32_len)
	deallocate(p1)
...
sub exe_32_64()
	...
	dim as ubyte ptr p1 = allocate(EXE32_64_len)
	p1 = Decrypt(EXE32_64, EXE32_64_len)
	memcpy(EXE32_64, p1, EXE32_64_len)
	deallocate(p1)
